...
H = p^2/2 + x^2. From this, we find the resulting eom
dx/dt = p, and dp/dt = -2x
...
def eoms(t: float, phase_point):
    ...
    return np.array([curr_momentum, -2 * curr_position])

#Gets data and puts it into two lists
ex_time_data = []
ex_position_data = []
file = open(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'Model-Generation','test1.txt')))
for line in file:
...
df.to_csv('first_trajectory.csv')