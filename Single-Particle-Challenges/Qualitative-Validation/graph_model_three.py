...
H = p^2/2 + x^6. From this, we find the resulting eom
dx/dt = p, and dp/dt = -6x^5
...
def eoms(t: float, phase_point):
    ...
    return np.array([curr_momentum, -6 * (curr_position**5)])

#Gets data and puts it into two lists
ex_time_data = []
ex_position_data = []
file = open(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'Model-Generation','test3.txt')))
for line in file:
...
df.to_csv('third_trajectory.csv')